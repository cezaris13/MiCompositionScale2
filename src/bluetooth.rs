...
pub async fn start_bluetooth_scanning(process_service_data_advertisement: &dyn Fn(PeripheralId, HashMap<Uuid, Vec<u8>>, &mut Vec<u8>)) {
    let central = get_adapter().await;

    let mut events = central.events().await.unwrap();

    central.start_scan(btleplug::api::ScanFilter::default()).await.unwrap();
