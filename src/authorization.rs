...
impl Authorization {
    ...
    pub async fn get_auth_token(&self, id: String, secret: String) -> Result<(), CliError> {
    ...
    pub fn write_auth_token(&self, token: Token) -> Result<(), CliError> {
        let json_token = serde_json::to_string(&token)?;
        let token_file: String = Utils::get_current_project_directory()? + "/" + TOKEN_FILE;
    ...
    pub fn file_exists(&self) -> Result<bool, CliError> {
        let token_file: String = Utils::get_current_project_directory()? + "/" + TOKEN_FILE;
        Ok(fs::metadata(token_file).is_ok())
    }

    pub fn read_auth_token(&self) -> Result<Token, CliError> {
        let token_file: String = Utils::get_current_project_directory()? + "/" + TOKEN_FILE;
    ...
    pub async fn get_access_token(&self) -> Result<String, CliError> {
    ...
    pub async fn refresh_access_token(&self) -> Result<String, CliError> {
        let refresh_token: String = self.read_auth_token()?.refresh_token;
        // let client_id: String = Utils::read_configuration_file()?.client_id;
        let client_id: String = String::from("");
        let client_secret: String = Utils::read_configuration_file()?.client_secret;
    ...
    pub fn is_access_token_expired(&self, access_token: &String) -> Result<bool, CliError> {
